...
CREATE TABLE IF NOT EXISTS affiliate_networks (
  id INT PRIMARY KEY AUTO_INCREMENT,
  name VARCHAR(100) NOT NULL UNIQUE,
  code VARCHAR(50) NOT NULL UNIQUE, -- 'amazon', 'cj', 'rakuten', 'shareasale'
  api_endpoint VARCHAR(255),
  api_key VARCHAR(255),
  api_secret VARCHAR(255),
  commission_rate DECIMAL(5,4) DEFAULT 0, -- 0.08 for 8%
  is_active BOOLEAN DEFAULT 1,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
-- Affiliate links table for managing affiliate URLs
CREATE TABLE IF NOT EXISTS affiliate_links (
  ...
  affiliate_code VARCHAR(255), -- affiliate tag/ID
...
-- Affiliate commissions table for tracking earnings
...
-- User subscriptions table for premium membership
CREATE TABLE IF NOT EXISTS user_subscriptions (
  ...
  price_id VARCHAR(255), -- for Stripe price IDs
...
-- Sponsored deals table for advertising platform
CREATE TABLE IF NOT EXISTS sponsored_deals (
  id INT PRIMARY KEY AUTO_INCREMENT,
  deal_id INT NOT NULL,
  advertiser_id VARCHAR(255) NOT NULL, -- could be user_id or external advertiser
  ...
  cpm DECIMAL(5,2), -- cost per thousand impressions
  start_date TIMESTAMP NOT NULL,
  end_date TIMESTAMP NOT NULL,
  status ENUM('pending', 'active', 'paused', 'completed', 'cancelled') DEFAULT 'pending',
  targeting_rules JSON, -- age, location, interests, etc.
...
-- Revenue analytics table for business intelligence
...
ALTER TABLE deals ADD COLUMN IF NOT EXISTS affiliate_network VARCHAR(50) NULL;
ALTER TABLE deals ADD COLUMN IF NOT EXISTS affiliate_commission DECIMAL(5,4) DEFAULT 0;
ALTER TABLE deals ADD COLUMN IF NOT EXISTS is_sponsored BOOLEAN DEFAULT 0;
ALTER TABLE deals ADD COLUMN IF NOT EXISTS sponsored_priority INT DEFAULT 0;

-- Add revenue fields to analytics_summary table
ALTER TABLE analytics_summary ADD COLUMN IF NOT EXISTS affiliate_commission DECIMAL(10,2) DEFAULT 0;
ALTER TABLE analytics_summary ADD COLUMN IF NOT EXISTS subscription_revenue DECIMAL(10,2) DEFAULT 0;
ALTER TABLE analytics_summary ADD COLUMN IF NOT EXISTS advertising_revenue DECIMAL(10,2) DEFAULT 0;
...
-- Products table for Amazon data normalization
CREATE TABLE IF NOT EXISTS products (
  id INT PRIMARY KEY AUTO_INCREMENT,
  external_product_id VARCHAR(50) NOT NULL, -- ASIN for Amazon
  store VARCHAR(50) NOT NULL DEFAULT 'amazon',
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  UNIQUE KEY unique_product (external_product_id, store),
  INDEX idx_category (category),
  INDEX idx_active (is_active),
  INDEX idx_store (store)
);

-- Price history for trend analysis and deal detection
CREATE TABLE IF NOT EXISTS price_history (
  ...
  mrp DECIMAL(10, 2),
  discount_percent INT DEFAULT 0,
  source VARCHAR(50) DEFAULT 'amazon_api',
  recorded_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (product_id) REFERENCES products(id) ON DELETE CASCADE,
  INDEX idx_product_time (product_id, recorded_at),
  INDEX idx_price_changes (product_id, price, recorded_at)
);

-- Add product relationship to deals table (backward compatible)
ALTER TABLE deals ADD COLUMN product_id INT NULL;
ALTER TABLE deals ADD CONSTRAINT fk_deal_product
  FOREIGN KEY (product_id) REFERENCES products(id) ON DELETE SET NULL;

-- Add indexes for Amazon-specific queries
ALTER TABLE deals ADD INDEX idx_product_id (product_id);
ALTER TABLE deals ADD INDEX idx_verified_created (verified, created_at);
